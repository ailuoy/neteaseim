...
class HttpClient
{
     ...
     * @return Client
     */
    public static function getGuzzleInstance($option = [])
    {
        if (!self::$guzzleInstance instanceof \GuzzleHttp\Client) {
            $defaultConfig        = ['timeout' => 2];
            self::$guzzleInstance = new Client(array_merge($defaultConfig, $option));
        }
    ...
    public static function send($method, $api, array $body)
    {
        ...
        $client = self::getGuzzleInstance($option);
        try {
            $response = $client->request($method, $api, ['body' => $body]);
